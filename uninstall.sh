...
if sudo grep -q "card0-eDP-2-backlight/brightness" /etc/sudoers; then
    sudo sed -i '\|card0-eDP-2-backlight/brightness|d' /etc/sudoers
fi
...
if [ "$KEEP_UI" = false ]; then
    ...
    rm -f "$TARGET_HOME/.config/autostart/zenbook-duo-control.desktop" 2>/dev/null || true
