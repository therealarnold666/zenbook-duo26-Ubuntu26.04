...
fn build_gnome_orientation_args(
    orientation: &Orientation,
    scale: f64,
    logical_count: usize,
) -> Vec<String> {
    let scale = format!("{scale:.6}");
    let (primary_transform, secondary_transform) = gnome_orientation_transforms(orientation);
    ...
    if logical_count > 1 {
        ...
        args.push(secondary_transform.to_string());
    }
...
"#;
    ...
    #[test]
    fn gnome_orientation_uses_flipped_secondary_baseline() {
        let args = build_gnome_orientation_args(&Orientation::Normal, 1.66, 2);
