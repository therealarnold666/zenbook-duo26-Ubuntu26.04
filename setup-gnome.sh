...
# Ubuntu 26.04 (GNOME 50 on Wayland) can hit login-loop/input races with
# aggressive early USB remap. Keep it opt-in there; users can re-enable later.
if [ -r /etc/os-release ]; then
    . /etc/os-release
    if [ "${ID:-}" = "ubuntu" ] && [ "${VERSION_ID:-0}" = "26.04" ]; then
        USB_MEDIA_REMAP_DEFAULT=false
...
case "${ENABLE_USB_MEDIA_REMAP_ANSWER}" in
    ...
    [nN]|[nN][oO])
        ...
        ;;
    *)
        USB_MEDIA_REMAP_ENABLED="${USB_MEDIA_REMAP_DEFAULT}"
        ;;
esac
