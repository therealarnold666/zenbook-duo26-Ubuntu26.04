...
- ASUS Zenbook Duo (USB vendor `0B05`, product `1B2C` or `1CD7`, depending on keyboard revision/model)
...
- `install-ui.sh` also creates `~/.config/autostart/zenbook-duo-control.desktop` by default so the UI starts automatically after login (use `--no-autostart` to skip)
- Dev mode:
